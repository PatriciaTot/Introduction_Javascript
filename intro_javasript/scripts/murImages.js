...
const init = () => {
  ...
  const images = dataImages.map( image => createImage(image) );
  const mur = document.getElementById('mur');
  images.forEach( image => mur.appendChild(image));
  images.forEach(image => image.addEventListener('mouseover', details));
